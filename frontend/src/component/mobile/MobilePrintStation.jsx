...
export default function MobilePrint() {
  ...
  async function getFinishedCabinet() {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_HOST}/api/scan/getCompleteCabinet/${cabinetLabel}`, {
        method: "GET",
      });
  ...
  function ValidateCabinet() {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_HOST}/api/scan/checkcabinet/${localStorage.getItem("workorder_id")}/${cabinet_label}`, {
      method: "POST",
    })
      .then((response) => {
        if (!response.ok) {
